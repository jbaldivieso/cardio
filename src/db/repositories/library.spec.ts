...
describe('library repository', () => {
  ...
  beforeEach(async () => {
    ...
    await db.folders.bulkAdd([
      { id: 'old-empty-folder', name: 'Japanese', createdAt: 1, updatedAt: 1 },
  ...
  describe('replaceAll', () => {
    it('loads the snapshot and drops everything that was there before', async () => {
      await library.replaceAll(snapshot)
    ...
    it('writes nothing at all when the snapshot cannot be loaded', async () => {
      ...
      await expect(library.replaceAll(duplicated)).rejects.toThrow()
    ...
    it('leaves nothing at all behind when given an empty snapshot', async () => {
      await library.replaceAll({ folders: [], decks: [], cards: [] })
